    ...
    for(n in 1:length(sample.true)) { #Sample size loop
      ...
      for(p in 1:length(theta.true)) {
        ...
        for(i in 1:n.iter) { #iteration loop
          ...
          for(j in 1:n.pt) {
            y.bin[j,] <- rmultinom(1, N.true[j], pi.true)
          } #close j loop
          ...
          data <- list(n.grid=samp.n, n.pt=n.pt, grid.pt.ind=grid.pt.ind, area.circle=area.circle, nG=nG, breaks=breaks,
                       area.band=area.band, area.prop=area.prop, nind=length(dclass), y.sum=y.sum, dclass=dclass)
          params <- c('lambda', 'theta.mn', 'pcap', 'p', 'D.all')
          inits <- list(list('theta.mn' = 400, 'log.lambda' = lambda.true, 'N.pt' = y.sum),
                        list('theta.mn' = 400, 'log.lambda' = lambda.true, 'N.pt' = y.sum),
                        list('theta.mn' = 400, 'log.lambda' = lambda.true, 'N.pt' = y.sum))
          model.out <- jagsUI(data = data, parameters.to.save = params, inits = inits,
                              ...
                              n.chains = 3, n.iter = 1000, n.burnin = 1000*0.5, n.thin = 1, parallel = TRUE, verbose = FALSE)
        ...
        cat('D = ',d,' of ',length(D.true),', Het =',h,' of ',length(Het.true),', n = ',n,' of ',length(sample.true),', p = ',p,' of ',length(theta.true),'\n',sep='')
...
#     D.RMSE.pct = sqrt(mean(D.pctbias^2)) * 100
#   )
...
sub.tab <- sum.table[which(sum.table$Heterogeneity==Het.true[1] & sum.table$theta==theta.true[1]),]
...
g.pbias.Het1.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.pctbias.median)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="Density % bias") + 
  ylim(ymin, ymax) +
...
sub.tab <- sum.table[which(sum.table$Heterogeneity==Het.true[2] & sum.table$theta==theta.true[1]),]
sub.tab$Density <- as.factor(sub.tab$Density)
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pbias.Het10.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.pctbias.median)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0) + 
  theme_bw() + 
...
sub.tab <- sum.table[which(sum.table$Heterogeneity==Het.true[1] & sum.table$theta==theta.true[2]),]
sub.tab$Density <- as.factor(sub.tab$Density)
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pbias.Het1.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.pctbias.median)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="Density % bias") + 
  ylim(ymin, ymax) +
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
...
sub.tab <- sum.table[which(sum.table$Heterogeneity==Het.true[2] & sum.table$theta==theta.true[2]),]
sub.tab$Density <- as.factor(sub.tab$Density)
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pbias.Het10.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.pctbias.median)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[2],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="Density % bias") + 
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.cov.Het1.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.cov)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0.95) + 
  ylim(0,1) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="Density coverage") + 
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
                                  size=14),  # X axis title
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.cov.Het10.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.cov)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0.95) + 
  ylim(0,1) + 
  theme_bw() + 
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.cov.Het1.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.cov)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0.95) + 
  ylim(0,1) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="Density coverage") + 
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
                                  size=14),  # X axis title
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.cov.Het10.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.cov)) + 
  geom_point(aes(col=Density)) + 
  geom_hline(yintercept = 0.95) + 
  ylim(0,1) + 
  theme_bw() + 
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.CV.Het1.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.CV.median)) + 
  geom_point(aes(col=Density)) + 
  geom_errorbar(aes(x=N.samp,ymin=D.CV.LCI,ymax=D.CV.UCI,width=0)) +
  coord_cartesian(ylim=c(0,1)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="Density CV") + 
  ylim(ymin, ymax) +
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.CV.Het10.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.CV.median)) + 
  geom_point(aes(col=Density)) + 
  geom_errorbar(aes(x=N.samp,ymin=D.CV.LCI,ymax=D.CV.UCI,width=0)) +
  coord_cartesian(ylim=c(0,1)) + 
  theme_bw() + 
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.CV.Het1.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.CV.median)) + 
  geom_point(aes(col=Density)) + 
  geom_errorbar(aes(x=N.samp,ymin=D.CV.LCI,ymax=D.CV.UCI,width=0)) +
  coord_cartesian(ylim=c(0,1)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="Density CV") + 
  ylim(ymin, ymax) +
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.CV.Het10.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.CV.median)) + 
  geom_point(aes(col=Density)) + 
  geom_errorbar(aes(x=N.samp,ymin=D.CV.LCI,ymax=D.CV.UCI,width=0)) +
  coord_cartesian(ylim=c(0,1)) + 
  theme_bw() + 
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.RMSE.Het1.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="RMSE") + 
  ylim(ymin, ymax) +
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.RMSE.Het10.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[2],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="RMSE") + 
  ylim(ymin, ymax) +
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.RMSE.Het1.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="RMSE") + 
  ylim(ymin, ymax) +
  #  theme(legend.position='None') +
  theme(axis.title=element_text(size=32),  # main title
        axis.title.x=element_text(vjust=00,
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.RMSE.Het10.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[2],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="RMSE") + 
  ylim(ymin, ymax) +
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pctRMSE.Het1.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE.pct)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="RMSE (%)") + 
  ylim(ymin, ymax) +
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pctRMSE.Het10.theta150 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE.pct)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[2],",\nDetection range = ", theta.true[1], " m",sep=''), x="Number of grid cells", y="RMSE (%)") + 
  ylim(ymin, ymax) +
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pctRMSE.Het1.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE.pct)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[1],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="RMSE (%)") + 
  ylim(ymin, ymax) +
...
sub.tab$N.samp[which(sub.tab$Density==D.true[1])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[1])] - 0.5
sub.tab$N.samp[which(sub.tab$Density==D.true[3])] <- sub.tab$N.samp[which(sub.tab$Density==D.true[3])] + 0.5
g.pctRMSE.Het10.theta55 <- ggplot(sub.tab, aes(x=N.samp, y=D.RMSE.pct)) + 
  geom_point(aes(col=Density)) + 
  theme_bw() + 
  labs(title=paste('Heterogeneity=',Het.true[2],",\nDetection range = ", theta.true[2], " m",sep=''), x="Number of grid cells", y="RMSE (%)") + 
  ylim(ymin, ymax) +
...
grid.arrange(g.pbias.Het1.theta150, g.pbias.Het10.theta150, g.pbias.Het1.theta55, g.pbias.Het10.theta55, nrow=2, ncol=2)
grid.arrange(g.cov.Het1.theta150, g.cov.Het10.theta150, g.cov.Het1.theta55, g.cov.Het10.theta55, nrow=2, ncol=2)
grid.arrange(g.CV.Het1.theta150, g.CV.Het10.theta150, g.CV.Het1.theta55, g.CV.Het10.theta55, nrow=2, ncol=2)
grid.arrange(g.RMSE.Het1.theta150, g.RMSE.Het10.theta150, g.RMSE.Het1.theta55, g.RMSE.Het10.theta55, nrow=2, ncol=2)
grid.arrange(g.pctRMSE.Het1.theta150, g.pctRMSE.Het10.theta150, g.pctRMSE.Het1.theta55, g.pctRMSE.Het10.theta55, nrow=2, ncol=2)
dev.off()
